...
# Client-1 is requesting a text file from the server. The server should simply check to see if it has the requested text file, if yes, it will send it to the client-1, otherwise the server has to  make a request to client-2 to upload the file to the server. As soon as the server has the file from client-2, then it will send it to client-1. As soon as client-1 receives the file, it should send a simple ACK to the server confirming the reception of the file. When the server receives the ACK from client-1, it should delete the file because it is not an existing file on the server and the sever borrowed it from the other client. Note that if the requested file is already an existing file on the server, then the file should not be deleted after the request is served. The file will only be deleted if the server has borrowed it from a client. Your program should handle all of these automatically.
...
def download(client_socket, params, SERVER_PATH):
        ...
        print(osp.getsize(osp.join(SERVER_PATH, params[0])))
...
# Server will be realized through a process accepting an incoming connection and handling the commands or files coming from the client. Handling two clients is required. 
# The server will be able to handle multiple clients, commands, and files at the same time.
#handle_client function will be used to handle the client connection. It will have commands CONNECT server_IP_address server_port, UPLOAD filename, DOWNLOAD filename, DELETE filename, and DIR: returns the content of folder
def handle_client(client_socket, client_address):
    ...
    while True:
        ...
        # If client1 wants to upload a file, upload it to the server directory
            ...
            print(f"Time taken to upload: {end - start}")
        # If client wants to download a file, upload it to the client directory. If it does not exist, ask client2 to upload the file to the server, and then send the file to the client, after that delete the file from the server
        elif entry == "DOWNLOAD":
            #make a timer to see how long it takes to download the file
            start = time.time()
            filename = params[0]
            SERVER_PATH = osp.join(pathlib.Path(__file__).parent.absolute(), "server")
            if osp.exists(osp.join(SERVER_PATH, params[0])):
                ...
                elif client_socket == connections[1]:
                    sending = f'UPLOAD2@{filename}'
                    connections[0].send(sending.encode(TYPE))
                    time.sleep(2)
                    print(filename)
            ...
            print(f"Time to download the file: {end - start}")
        ...
        # in a table format that has "Name", "Size (in MB)", "Upload Date & Time", "Number of Downloads",
            ...
            print(files_info)
            table = tabulate(files_info, headers=["Name", "Size (in bytes)", "Upload Date & Time", "Number of Downloads"])
            print(table)
