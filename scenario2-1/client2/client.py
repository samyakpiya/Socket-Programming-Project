...
def uploadToServer(filename, client):
    ...
    if osp.exists(filename):
        ...
        #print time it took to upload and the download speed in MB/s
        print(f"[CLIENT] Upload complete in {end - start + 2} seconds, with the rate of {(osp.getsize(filename)/(end - start))/1000000} MB/s")
        ...
        if command.lower() == "hello":
            ...
            if (len(params) == 2):
                if osp.exists(params[0]):
                    ...
                    client.send(s.encode(FORMAT))
                time.sleep(2)
                if osp.exists(params[1]):
                    print(f"[CLIENT] Uploading '{params[1]}'")
                    s = f"UPLOAD@{params[1]}@{osp.getsize(params[1])}"
                    client.send(s.encode(FORMAT))
